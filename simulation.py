...
class MPC():
    ...
    def mpc_controller(self, x_0, N, dt):
        ...
        # Extract the state and input solutions
        state_solution = sol_x[:ns] 
        input_solution = sol_x[ns * (N + 1): ns * (N + 1) + 3]
...
for i in range(steps):
    control_input = controller.mpc_controller(prev_state, horizon, dt)

    result = control_input['x'].full().flatten()
    states_x = result[0:9*(horizon+1):9]
    states_y = result[1:9*(horizon+1):9]
    inputs = result[9*(horizon+1):]
